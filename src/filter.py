class Filter:
    def __init__(self):
        self.whitelist = [
            ...
            # canadian universities
                        'ubc.ca', 'uwaterloo.ca', 'utoronto.ca', 'mcmaster.ca', 'queensu.ca', 'ucalgary.ca',
            'ualberta.ca',
    ...
    def filter(self, all_results: list, args: dict) -> list:
        filtered = list()
        for result_list in all_results:
            if isinstance(result_list[-1], str) and result_list[-1].upper() == 'TRUSTED':
                filtered.append(result_list[:-1])
            else:
                filtered.append(self.filter_results(result_list, args))
        return self.combine_results(filtered)

    # takes in a list of dictionaries
    def filter_results(self, results: list, args: dict) -> list:
        filtered = list()
        valid = False
        for result in results:
            if 'from_sites' in args:
                ...
                if any(x in result['url'] for x in self.whitelist):
                    valid = True
    ...
    def combine_results(self, all_results: list) -> list:
        unique_urls = set()
        combined = list()
        for result_list in all_results:
